...
const Orders = () => {
  ...
  const loadOrders = async () => {
    const response = await api.get('/operations/orders');
  ...
  const handleSubmit = async (event: FormEvent) => {
    ...
    if (!branchId || cartItems.length === 0) {
      setMessage('Please select a branch and add at least one menu item.');
      return;
    }
              ...
              <tbody className="divide-y divide-slate-200 bg-white">
                {orders.map((order) => (
                  <tr key={order.id}>
                    ...
                    <td className="px-6 py-4 text-sm text-slate-500">{order.branch?.name}</td>
                    <td className="px-6 py-4 text-sm text-slate-500">{order.customer?.name || 'Guest'}</td>
                    <td className="px-6 py-4 text-sm text-slate-500">{order.status}</td>
                    <td className="px-6 py-4 text-sm text-slate-500">${order.total?.toFixed(2)}</td>
                  </tr>
                ))}
              </tbody>
            </table>
